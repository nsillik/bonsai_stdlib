...
// NOTE(nsillik): Flushes stdout and the log.txt mirror of it.  Called before a trap, so the tail
// of the log is not stuck in a buffer that a SIGTRAP/SIGSEGV will take with it.
link_internal void
FlushStdout();
